...
async function publishAsAdmin(form){
  try{
    ...
    let newJob;
    if(window.LDFSupabaseJobs?.createJob){newJob=await window.LDFSupabaseJobs.createJob(data);}else{
      newJob={id:Date.now(),...data,access:0,max:(typeof MAX_WORKERS_PER_JOB!=='undefined'?MAX_WORKERS_PER_JOB:6),cost:(typeof workerUnlockPrice==='function'?workerUnlockPrice(data.budget):0),unlockedBy:[],status:'activ',ownerId:currentUser.id,workflowStatus:'disponibila'};
    }
    ...
    form.reset();
    if(typeof toast==='function')toast('Lucrarea a fost publicată gratuit cu succes.');
    if(typeof nav==='function')nav('jobs');
    if(typeof renderJobs==='function')renderJobs();
    paintAdminBadge();
  }catch(err){
    try{if(typeof currentUser!=='undefined'&&currentUser)currentUser.role='admin';}catch(_e){}
    console.error(err);if(typeof toast==='function')toast(err?.message||'Nu am putut publica lucrarea.');
  }
}
